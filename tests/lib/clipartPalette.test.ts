...
import { paletteFromColors } from "@/ai/style";
...
describe("styleWithPalette", () => {
  ...
  it("builds a complete, printable style for a set that has none yet", () => {
    ...
    expect(next).toEqual({
      template: "collage",
      font: DEFAULT_FONT,
      palette: paletteFromColors(["#ff0000", "#00ff00"], i.shirtColor),
  ...
  it("derives the palette through the one shared rule, so upload and AI-fallback cannot drift", () => {
    ...
    expect(styleWithPalette(null, i, "/c.png", colors).palette).toEqual(paletteFromColors(colors, i.shirtColor));
    expect(styleWithPalette(tuned, i, "/c.png", colors).palette).toEqual(paletteFromColors(colors, i.shirtColor));
  });
