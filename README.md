...
- Clipart is AI-generated from the theme (Z.ai CogView) or uploaded by hand. Either way it goes
  through `removeBackground()`, which floods in from the border to lift a flat backdrop and leaves
  white *inside* the drawing — an eye, a tooth, a gleam — alone.
...
- The editor loads only the font faces a set can draw with. `setFonts()` reads the families out of
  `style.font` and the members' layer overrides — both plain data, available before anything is
  measured — and `useBrowserMeasurer(families)` withholds the measurer until exactly those faces are
  in the browser. A design on the default Fredoka now waits on ~90 KB rather than the registry's
  1.4 MB. Changing font in the inspector loads the new face while the previous measurer keeps
  working, so the canvas does not blank. `src/app/fonts.css` still declares all nine `@font-face`
  rules, and that costs nothing: a rule is a promise, not a download — the font picker's own
  previews are what pull the rest, lazily and off the critical path.
- `removeBackground()` only lifts a background that is **flat and touches the border**. A clipart on
  a gradient or a photographic backdrop is handed back untouched, by design: `isBackgroundRemovable()`
  refuses when the border ring is not one colour, because guessing there tears holes in the picture.
  Its two thresholds are a shop judgement, not a fact — retune them against real clipart. A clipart
  supplied by a batch CSV's `clipart_url` does **not** go through it; only uploads and AI generations
  do.
