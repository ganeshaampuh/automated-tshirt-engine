...
export const MemberSchema = z.object({
  ...
  overrides: z.record(z.string(), LayerSchema.partial()).optional(), // keyed by layer id
});
...
Slot geometry (fractions of canvas side `S`, safe area `A`), tuned to the reference sample `docs/samples/unicorn-ayah.png`:
...
```ts
import type { Design, Member, Set } from "./types";
...
export function applyOverrides(design: Design, overrides: Member["overrides"]): Design {
  ...
  return { ...design, layers: design.layers.map(l => (overrides[l.id] ? ({ ...l, ...overrides[l.id] } as typeof l) : l)) };
}
...
sharp("docs/samples/unicorn-ayah.png")
...
Run: `pnpm test tests/engine/render.test.ts`
Expected: pass (goldens created on first run). Open `tests/engine/__golden__/collage-en-ayah.png` and compare with `docs/samples/unicorn-ayah.png`: numeral left, unicorn right overlapping, name top, "Ayah" bottom. If proportions are clearly off, adjust the geometry table in Task 6, rerun with `UPDATE_GOLDEN=1 pnpm test`, and re-inspect. Commit goldens only when they look right.
