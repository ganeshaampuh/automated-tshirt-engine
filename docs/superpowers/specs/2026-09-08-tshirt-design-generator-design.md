...
- One print area: front chest, 4500×5400 px @ 300 DPI (15×18 in), transparent background
...
type Design = {
  version: 1;
  canvas: { w: 4500; h: 5400; dpi: 300 };     // fixed for v1
...
`scale = displayWidth / 4500`. Validated with Zod; the same schema validates
...
- **Archetypes** (`archetypes/*.ts`): `layout(input) → Design`. Input is the
  text, image metadata (w, h), shirt color, and the LLM's choices (§6). Each
  archetype computes exact positions inside a safe area (5% inset from the
  canvas edge), so nothing can be placed off the print area.
...
- **Mockup compositor** (`mockup.ts`): draws a design onto a shirt photo.
  Shirt assets are white-base PNGs with metadata `{ printArea: {x,y,w,h},
  perspective?: [...] }`. Shirt color is applied by tinting the white base;
  the design is drawn into `printArea` with `multiply` blend so fabric
  texture shows through. Used by both the editor and server previews.
...
Input: `{ text, shirtColor, imageMeta | null, note?: string }`.
...
Three panels:
- **Left, inputs**: text field, image (upload dropzone **or** prompt +
...
- **Right, inspector**: selected layer's properties (font, weight, size,
  color, spacing, align). Every change writes to the Design JSON; Konva is a
  view of it, never the source of truth.
...
required), optional `shirt_color` (hex or name, default white), `sku`, `note`.
Max 500 rows in v1.
...
designs
  ...
  source_text, source_image_url?, source_image_prompt?, shirt_color, note?
...
- Shirt mockups: 2 base photos in v1 (flat lay, on-model), white base, with
  `printArea` metadata in a JSON sidecar.
...
- **Archetypes / textFit**: unit tests with long text, one word, empty image,
  extreme aspect ratios; assert every layer within safe area.
...
2. Engine: Design schema, archetypes, textFit, server renderer + golden tests
3. Mockup compositor
